...
const Sidebar = ({ isOpen }) => {
  return (
    <div
      id="sidebar"
      className={`fixed left-0 top-0 z-10 h-full w-64 overflow-y-scroll transform bg-white transition-all duration-300 ease-in-out dark:bg-brandDark-950 dark:text-brandDark-100 md:hidden ${isOpen ? "translate-x-0" : "-translate-x-full"
        }`}
    >
      <nav className="mt-16 flex flex-col items-start space-y-4 p-6">
        ...
        <NavLink to="/pdflist" className="flex gap-2 w-full rounded-lg p-2 dark:text-brandDark-100">
          Resumes
        </NavLink>
      </nav>
    </div>
